...
ngOnInit(): void {
  ...
  this.audioService.play('http://localhost:3000/public/audio/normalLab.mp3', 'BGMusic', true)
}
...
start():void{
  this.fadeOut=true;
  this.audioService.play('http://localhost:3000/public/audio/buttonStart.mp3', 'SFX', false)
 if(this.maxLevel===0) {
  setTimeout(()=>{
  ...
  if (target && target.value !== null) {
    ...
    switch (type) {
      ...
      case 'BGAudio':
        ...
        this.audioService.playOnceForDuration('http://localhost:3000/public/audio/crowd.mp3', 3200, 'BGAudio')
        break;
      case 'SFX':
...
ngOnDestroy(): void {
  this.audioService.stop('http://localhost:3000/public/audio/normalLab.mp3')
}
