...
export class LevelsComponent implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit{
  ...
  // logica del livello generale
    ...
    correctImage:string="http://localhost:3000/public/images/correct.gif";
  ...
  ngOnInit(): void {
    ...
    ngAfterViewInit(): void {
      if (this.isPlatFormBrowser) {
        this.img = document.getElementById('findImage') as HTMLImageElement;
        if (this.img) {
  ...
  findClick(event:MouseEvent, area:any):void {
      ...
      if (!area.founded) {
        ...
        setTimeout(() => {
          ...
          this.correctImage = `http://localhost:3000/public/images/bglitch.gif?${new Date().getTime()}`;
          this.correctVisible = true;
          this.audioService.play(this.images.foundSFX.src,this.images.foundSFX.type, false)
          setTimeout(() => {
            this.correctVisible = false;
  ...
  calculateCenter(area: any) {
    ...
    area.centerX = centerX - width/2.3;
    area.centerY = centerY -height/3.2 ;

}
  activateZoom():void{
    this.usedZoom=true;
    if(this.zoomActive===false){
      this.audioService.play('http://localhost:3000/public/audio/zoomin.mp3', 'SFX', false);
      this.npcLine=this.npcLines[7]
      this.speechVisible=true;
      this.zoomActive=true;
    }else if(this.zoomActive===true){
      this.audioService.play('http://localhost:3000/public/audio/zoomout.mp3', 'SFX', false);
